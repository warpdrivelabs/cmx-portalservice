...
function extractMeta (file) {
  ...
  return { html: html.slice(0, html.indexOf('<script type="application/json" id="__designer_meta__">')), meta: JSON.parse(html.slice(start, end)) };
}
...
const REFRESH_CHROME_BODY = CHROME_HELPERS + `
function refreshHeaderChrome(ms){
  var ds=headersDs(ms); if(!ds){ setText('hdrPos',''); return; }
  var i=(ds.cursorIndex==null?-1:ds.cursorIndex);
  setText('hdrPos',(i+1)+'/'+ds.rows.length);
...
const CONTENT_CSS = `
    ...
    .fico-hdr-hero { display:flex; flex-wrap:wrap; align-items:flex-end; gap:12px 16px; padding:9px 12px; border-bottom:1px solid var(--fico-border); background:linear-gradient(105deg, color-mix(in srgb, var(--fico-cyan) 6%, var(--sapList_Background,#fff)), color-mix(in srgb, var(--fico-violet) 4%, transparent) 55%, transparent); }
    .fico-seal-strip { display:flex; align-items:flex-end; justify-content:center; flex-shrink:0; width:56px; min-height:56px; }
    .fico-seal { width:52px; height:52px; border-radius:50%; display:none; align-items:center; justify-content:center; position:relative; transform:rotate(-10deg); color:var(--fico-draft); background:color-mix(in srgb, currentColor 8%, var(--sapList_Background,#fff)); border:2px double currentColor; box-shadow:inset 0 0 0 1px color-mix(in srgb, currentColor 40%, transparent), 0 3px 12px color-mix(in srgb, currentColor 22%, transparent); user-select:none; }
    ...
    .fico-seal::before { content:''; position:absolute; inset:4px; border-radius:50%; border:1px solid color-mix(in srgb, currentColor 55%, transparent); pointer-events:none; }
    .fico-seal::after { content:''; position:absolute; inset:8px; border-radius:50%; border:1px dashed color-mix(in srgb, currentColor 30%, transparent); pointer-events:none; }
    .fico-seal-text { display:flex; flex-direction:column; align-items:center; font-size:0.62rem; font-weight:900; line-height:1.05; letter-spacing:0.12em; font-family:"STKaiti","KaiTi","Songti SC",serif; z-index:1; }
    .fico-hdr-main { flex:1 1 200px; min-width:0; display:flex; flex-direction:column; justify-content:flex-end; align-self:flex-end; gap:4px; }
    .fico-hdr-title-row { display:flex; flex-direction:column; align-items:flex-start; justify-content:flex-end; gap:3px; min-width:0; width:100%; min-height:56px; box-sizing:border-box; }
    .fico-hdr-headband { width:100%; min-width:0; }
    .fico-hdr-title { margin:0; font-size:1.25rem; font-weight:800; letter-spacing:0.04em; line-height:1.15; color:var(--sapTitleColor,#223548); }
    .fico-hdr-pos { font-size:0.72rem; color:var(--sapContent_LabelColor,#6a6d70); margin-left:6px; line-height:1; font-variant-numeric:tabular-nums; font-family:ui-monospace,SFMono-Regular,Menlo,monospace; white-space:nowrap; }
    .fico-hdr-nav, .fico-line-nav { display:flex; align-items:center; gap:2px; flex-shrink:0; }
    .fico-hdr-nav ui5-button, .fico-line-nav ui5-button { width:var(--fico-nav-btn-size); height:var(--fico-nav-btn-size); min-width:var(--fico-nav-btn-size); min-height:var(--fico-nav-btn-size); padding:0; box-sizing:border-box; --sapButton_Height:var(--fico-nav-btn-size); --_ui5_button_base_min_width:var(--fico-nav-btn-size); --_ui5_button_base_min_height:var(--fico-nav-btn-size); }
    .fico-hdr-act-wrap { flex-shrink:0; margin-right:6px; }
    .fico-act-rail { display:flex; align-items:center; gap:0; padding:1px 4px; border-radius:7px; background:color-mix(in srgb, var(--sapList_Background,#fff) 68%, transparent); border:1px solid color-mix(in srgb, var(--fico-cyan) 22%, var(--fico-border)); box-shadow:inset 0 0 10px color-mix(in srgb, var(--fico-cyan) 5%, transparent), inset 0 1px 0 var(--fico-edge-highlight); }
    .fico-util-chip { --util-hue:var(--fico-cyan); position:relative; display:flex; align-items:center; justify-content:center; width:var(--fico-nav-btn-size); height:var(--fico-nav-btn-size); margin:0 1px 0 0; padding:0; border:none; border-radius:5px; cursor:pointer; background:transparent; color:var(--util-hue); font-size:0.62rem; font-weight:900; font-family:ui-monospace,monospace; }
    ...
    .fico-act-chip { --act-hue:var(--fico-cyan); position:relative; display:flex; align-items:center; justify-content:center; width:calc(var(--fico-nav-btn-size) + 2px); height:var(--fico-nav-btn-size); margin:0; padding:0 1px; border:none; border-radius:5px; cursor:pointer; background:transparent; color:var(--act-hue); font-family:ui-monospace,monospace; }
    ...
    .fico-act-glyph { position:relative; z-index:1; font-size:0.78rem; font-weight:900; line-height:1; letter-spacing:0.06em; font-family:"STKaiti","KaiTi","Songti SC",serif; background:linear-gradient(160deg, var(--act-hue), color-mix(in srgb, var(--act-hue) 42%, var(--sapTextColor,#fff))); -webkit-background-clip:text; background-clip:text; -webkit-text-fill-color:transparent; filter:drop-shadow(0 0 4px color-mix(in srgb, var(--act-hue) 40%, transparent)); }
    ...
    .fico-kpi-row { display:flex; align-items:flex-end; gap:8px; flex-shrink:0; flex-wrap:wrap; align-self:flex-end; }
    .fico-kpi { min-width:88px; padding:6px 10px; border-radius:9px; background:color-mix(in srgb, var(--sapList_Background,#fff) 78%, transparent); border:1px solid var(--fico-border); text-align:center; display:flex; flex-direction:column; justify-content:center; gap:2px; }
    .fico-kpi-lbl { font-size:0.62rem; text-transform:uppercase; letter-spacing:0.07em; color:var(--sapContent_LabelColor,#6a6d70); white-space:nowrap; }
    .fico-kpi-val { font-size:0.98rem; font-weight:800; font-variant-numeric:tabular-nums; font-family:ui-monospace,SFMono-Regular,Menlo,monospace; color:var(--sapTitleColor,#223548); line-height:1.3; }
    .fico-kpi-val.fico-ok { color:var(--fico-mint); }
    .fico-kpi-val.fico-warn { color:var(--fico-warn); }
    .fico-kpi-status { min-width:96px; }
    .fico-dc-ring-wrap { display:flex; flex-direction:column; align-items:center; justify-content:center; gap:3px; padding:4px 6px; min-width:44px; }
    #dcMixRing { width:28px; height:28px; border-radius:50%; padding:2px; background:conic-gradient(var(--fico-cyan) 0% 50%, var(--fico-violet) 50% 100%); box-sizing:border-box; }
    #dcMixRing::after { content:''; display:block; width:100%; height:100%; border-radius:50%; background:var(--sapList_Background,#fff); }
    .fico-dc-badge { font-size:0.58rem; font-weight:800; letter-spacing:0.08em; color:var(--fico-cyan); font-family:ui-monospace,monospace; }
    .fico-hdr-form { display:block; flex:0 0 auto; padding:2px 4px 4px; }
    ...
    @media (max-width:720px) { .fico-seal{width:44px;height:44px;} .fico-kpi-row{width:100%;justify-content:space-between;} .fico-hdr-nav{flex-wrap:nowrap;overflow-x:auto;max-width:100%;} }
`;
      ...
      <div class="fico-hdr-main">
        <div class="fico-hdr-title-row">
          ...
          <div class="fico-hdr-nav">
            ...
            <span id="hdrPos" class="fico-hdr-pos" aria-label="当前凭证序号"></span>
          </div>
        </div>
      </div>
...
// ── Patch content meta (保留现有逻辑，仅增量) ──
const { meta: contentMeta } = extractMeta('fico-ws-content.html');

function upsertFn (name, params, tailBody) {
  const existing = contentMeta.pageFns.find(f => f.name === name);
  const body = existing ? existing.body : '';
  const prefixEnd = body.indexOf('function headersDs');
  const sharedPrefix = prefixEnd >= 0 ? body.slice(0, prefixEnd) : body;
  const newBody = sharedPrefix + CHROME_HELPERS + tailBody;
  if (existing) {
    existing.body = newBody;
    if (params) existing.params = params;
  } else {
    contentMeta.pageFns.push({ name, params: params || '', body: newBody, readsVars: [], writesVars: [] });
  }
}

// navHeader: 保留导航逻辑，末尾改调 refreshHeaderChrome
const navFn = contentMeta.pageFns.find(f => f.name === 'navHeader');
if (navFn) {
  navFn.body = navFn.body
    .replace(/function refreshHdrPos\(ms\)\{[\s\S]*?\}\n\n/, CHROME_HELPERS + `
function refreshHeaderChrome(ms){
  var ds=headersDs(ms); if(!ds){ setText('hdrPos',''); return; }
  var i=(ds.cursorIndex==null?-1:ds.cursorIndex);
  setText('hdrPos',(i+1)+'/'+ds.rows.length);
...
`)
    .replace('refreshHdrPos(ms);', 'refreshHeaderChrome(ms);');
}

// initPage: 保留 applyBatch 等，仅把 refreshHdrPos 换成 refreshHeaderChrome
const initIface = contentMeta.pageInterfaces.find(i => i.name === 'initPage');
if (initIface) {
  if (!initIface.body.includes('function refreshHeaderChrome')) {
    initIface.body = initIface.body
      .replace(/function refreshHdrPos\(ms\)\{[\s\S]*?\}\n\n/, CHROME_HELPERS + `
function refreshHeaderChrome(ms){
  var ds=headersDs(ms); if(!ds){ setText('hdrPos',''); return; }
  var i=(ds.cursorIndex==null?-1:ds.cursorIndex);
...
`);
  ...
  initIface.body = initIface.body.replace(/refreshHdrPos\(ms\)/g, 'refreshHeaderChrome(ms)');
}

// 新增 pageFns（不覆盖已有 onLine/showHeaderDetail/postVoucher）
const newFns = [
  ['refreshHeaderChrome', 'ms', REFRESH_CHROME_BODY],
  ['navLine', 'level,dir', `
...
for (const [name, params, tail] of newFns) {
  if (!contentMeta.pageFns.find(f => f.name === name)) {
    const navBody = contentMeta.pageFns.find(f => f.name === 'navHeader');
    const prefix = navBody ? navBody.body.split('function headersDs')[0] : '';
    contentMeta.pageFns.push({ name, params, body: prefix + CHROME_HELPERS + tail, readsVars: [], writesVars: [] });
  }
}

// getPageApi: 增量暴露新 API，保留原有
...
// Explorer: 只换 HTML，meta 完全保留
const { meta: explorerMeta } = extractMeta('fico-ws-explorer.html');
writePage('fico-ws-explorer.html', EXPLORER_HTML, explorerMeta);

// Model: 仅加 __icon，不动其他
let modelRaw = fs.readFileSync(path.join(base, 'fico-ws-model.html'), 'utf8');
const iconOld = "__batchRows=host.__batchIndex.map(function(bi){ return { id:bi.batch_no, batch_name:bi.batch_name, __sub:(bi.company_code_id||'')+' · '+(bi.count||0)+' 张', __idx:bi.idx }; });";
const iconNew = "__batchRows=host.__batchIndex.map(function(bi){ return { id:bi.batch_no, batch_name:bi.batch_name, __sub:(bi.company_code_id||'')+' · '+(bi.count||0)+' 张', __idx:bi.idx, __icon:'batch-payments' }; });";
if (modelRaw.includes(iconOld) && !modelRaw.includes("__icon:'batch-payments'")) {
  modelRaw = modelRaw.replace(iconOld, iconNew);
  fs.writeFileSync(path.join(base, 'fico-ws-model.html'), modelRaw);
}

console.log('restored skins, preserved meta logic');
